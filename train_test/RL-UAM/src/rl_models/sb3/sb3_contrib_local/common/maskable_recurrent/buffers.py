from typing import Generator, NamedTuple, Optional, Union, Tuple, Callable
...
class MaskableRecurrentRolloutBuffer(RolloutBuffer):
    ...
    def _get_samples(
        ...
        return MaskableRecurrentRolloutBufferSamples(
            ...
            done_mask=self.pad_and_flatten(np.ones_like(self.returns[batch_inds])),
...
class MaskableRecurrentDictRolloutBuffer(DictRolloutBuffer):
    ...
    def add(self, *args, lstm_states: RNNStates, action_masks: Optional[np.ndarray], **kwargs) -> None:
        """
        :param hidden_states: LSTM cell and hidden state
        """
        self.hidden_states_pi[self.pos] = np.array(lstm_states.pi[0].cpu().numpy())
